        ...
        while True:
            ...
            for i in range(self.n_acts - 1):
                for j in range(self.mj_model.nu):
                    ...
                    from_pos = np.array([0.0, 0.0, 0.0], dtype=np.float64)
                    to_pos = np.array([1.0, 1.0, 1.0], dtype=np.float64)

                    mujoco.mjv_connector(geom, mujoco.mjtGeom.mjGEOM_CAPSULE, 0.02, from_pos, to_pos)
